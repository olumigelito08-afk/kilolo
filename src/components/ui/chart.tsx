...
>((props, ref) => {
  ...
  const payload = (props as unknown as { payload?: TooltipPayloadItem[] }).payload;
  const label = (props as unknown as { label?: string | number }).label;
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>
          {(labelFormatter as unknown as (a: unknown, b: unknown) => React.ReactNode)(
            value,
            payload,
          )}
        </div>
    ...
    >
      {(payload as LegendPayloadItem[])
        ...
        .map((item: LegendPayloadItem, index: number) => {
          ...
          return (
            <div
              key={`${item.dataKey ?? item.name ?? "legend"}-${index}`}
              className={cn(
